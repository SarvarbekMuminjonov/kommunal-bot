...
numberScene.action(/.+/, (ctx) => {
    ...
    if (ctx.match[0] == '0' && editing == '') {
        editing = ''
        ctx.answerCbQuery(`Hisob raqami 0 bilan boshlanmaydi`)
    }
    ...
    if (ctx.match[0] == 'natija') {
        ...
        const creditials = {
            personal_account: Number.parseInt(ctx.session.personalNumber),
            service_id: 3,
        ...
        getUserData(creditials).then(res => {
            res.forEach(val => resultMsg += `${val.name} : ${val.value}\n`)
            ctx.deleteMessage()
            ctx.reply(resultMsg)
        })
