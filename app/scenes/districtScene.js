...
const buttonGenerator = require("../utils/keyboards/buttuonGenerator");
const json = require("../constants/regionsEAC.json");
...
districtScene.enter((ctx) => {
	...
	ctx.reply("Tuman yoki shahringizni tanlang", {
		...buttonGenerator(
			ctx.session.regIndex,
			arr[ctx.session.regIndex].Children.Area.length
		),
	});
});
